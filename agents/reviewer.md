...
- To check a citation, resolve it: `openalex works get <doi-or-id>` (through `Bash`) confirms the work and pulls its abstract and metadata; read the actual source with `kagi_extract` or `WebFetch`, and use `kagi_search_fetch` (Academic lens `lens_id: "2"`) to find missing or stronger references.
