...
## Source verification
Verify links by reading them: `kagi_extract` (or `WebFetch`) fetches a page, and `kagi_search_fetch` hunts for a replacement. For any paper or DOI, `openalex works get <doi-or-id>` (through `Bash`) confirms the work exists, resolves its canonical metadata, and surfaces an open-access link when the cited URL is dead.
