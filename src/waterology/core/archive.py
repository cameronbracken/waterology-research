...
def export_run_crate(start: Path, run_id: str, destination: str, *, validate: bool = True) -> Path:
    from waterology.core.rocrate import export_crate

    return export_crate(start, run_id, destination, validate=validate)


def auto_export_run_crate(start: Path, run_id: str) -> Path:
    from waterology.core.rocrate import automatic_crate

    return automatic_crate(start, run_id)
