...
Requirements:
- Before starting, outline the watch plan: what to monitor, which signals matter, what counts as a meaningful change, and a sensible check frequency. Write it to `outputs/.plans/<slug>.md`. Summarize briefly and continue immediately unless the user asked to review the plan first.
...
- Save exactly one baseline artifact to `outputs/<slug>-baseline.md`, ending with a `Sources` section of direct URLs for every source used.
