...
Requirements:
- Before starting, outline the comparison plan: which sources to compare, which dimensions to evaluate, expected output structure. Write it to `docs/.plans/<slug>.md`. Summarize briefly and continue immediately unless the user asked to review the plan first.
...
- Save exactly one comparison to `docs/<slug>-comparison.md`, ending with a `Sources` section of direct URLs for every source used.
