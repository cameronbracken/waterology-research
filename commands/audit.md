...
Requirements:
- Before starting, outline the audit plan: which paper, which repo, which claims to check. Write it to `outputs/.plans/<slug>.md`. Summarize briefly and continue immediately unless the user asked to review the plan first.
...
- Save exactly one audit artifact to `outputs/<slug>-audit.md`, ending with a `Sources` section containing paper and repository URLs.
